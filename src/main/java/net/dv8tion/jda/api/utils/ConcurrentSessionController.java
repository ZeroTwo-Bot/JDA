    ...
    private static class Worker implements Runnable {
        ...
        public void run() {
            try {
                while (!queue.isEmpty()) {
                    processQueue();
                    // We always sleep here because its possible that we get a new session request before the rate
                    // limit expires
        ...
        private void processQueue() throws InterruptedException {
            ...
            try {
                ...
                node.run(false); // we don't use isLast anymore because it can be a problem with many reconnecting
                // shards
            } catch (NoSuchElementException ignored) {/* This means the node was removed before we started it */} catch (InterruptedException e) {
                queue.add(node);
                throw e;
                ...
                } else if (e instanceof ErrorResponseException && e.getCause() instanceof IOException) { /* This is
                already logged by the Requester */
